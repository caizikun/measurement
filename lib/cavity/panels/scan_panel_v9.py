...
class ScanGUI(QtGui.QMainWindow):
    def __init__(self, exp_mngr):
        ...
        self._exp_mngr = qt.instrument['cavity_experiment_manager']
        self._scan_mngr = qt.instruments['cavity_scan_manager']
    ...
    def autosave (self, state):
        if state == QtCore.Qt.Checked:
            self._scan_mngr.set_autosave(True)
        else:
            self._scan_mngr.set_autosave(False)

    def autostop (self, state):
        if state == QtCore.Qt.Checked:
            self._scan_mngr.set_autostop(True)
        else:
            self._scan_mngr.set_autostop(False)

    def montana_sync (self, state):
        if state == QtCore.Qt.Checked:
            self._scan_mngr.set_use_sync(True)
        else:
            self._scan_mngr.set_use_sync(False)

    def set_avg (self, value):
        '''
        this function seems to be a double of the 'msync_nr_avg_scans'. remove either.
        '''
        self._scan_mngr.set_nr_avg_scans(value)
        self.ui.sb_nr_scans_msync.setValue(value)

    def set_file_tag (self, text):
        self._scan_mngr.set_file_tag(str(text))

    def save_single(self):
        self._scan_mngr.save(data_index = '_single')
    ...
    def set_minV_lengthscan(self, value):
        self._scan_mngr.set_minV_lengthscan(value)

    def set_maxV_lengthscan (self, value):
        self._scan_mngr.set_maxV_lengthscan(value)

    def set_nr_steps_lengthscan (self, value):
        self._scan_mngr.set_nr_steps_lengthscan(value)

    def set_minV_finelaser (self, value):
        self._scan_mngr.set_minV_finelaser(value)

    def set_maxV_finelaser (self, value):
        self._scan_mngr.set_maxV_finelaser(value)

    def set_nr_steps_finelaser (self, value):
        self._scan_mngr.set_nr_steps_finelaser(value)

    def set_wait_cycles (self, value):
        self._scan_mngr.set_wait_cycles(value)  
    ...
    def set_min_lambda (self, value):
        self._scan_mngr.set_minlambda_lrlaser(value)

    def set_max_lambda (self, value):
        self._scan_mngr.set_maxlambda_lrlaser(value)

    def set_nr_calib_pts (self, value):
        self._scan_mngr.set_nr_calib_pts_lrlaser(value)

    def set_steps_lr_scan (self, value):
        self._scan_mngr.set_nr_steps_lrlaser(value)

    def set_min_msyncdelay(self,value):
        self._scan_mngr.set_min_msyncdelay(value)

    def set_max_msyncdelay(self, value):
        self._scan_mngr.set_max_msyncdelay(value)
    ...
    def set_delay_msync (self, value):
        self._scan_mngr.set_sync_delay_ms(value)

    def set_nr_repetitions (self,value):
        self._scan_mngr.set_nr_repetitions(value)

        ############### start and stop functions
    def start_lengthscan(self):
        self._scan_mngr.start_lengthscan()

    def stop_lengthscan(self):
        self._scan_mngr.stop_lengthscan()

    def start_finelaser(self):
        self._scan_mngr.start_finelaser()

    def stop_finelaser(self):
        self._scan_mngr.stop_finelaser()
 
    def start_sweep_msyncdelay(self):
        self._scan_mngr.start_sweep_msyncdelay()

    def stop_sweep_msyncdelay(self):
        self._scan_mngr.stop_sweep_msyncdelay()

    def start_lr_scan(self):
        self._scan_mngr.start_lr_scan()

    def resume_lr_scan(self):
        self._scan_mngr.resume_lr_scan()

    def stop_lr_scan(self):
        self._scan_mngr.stop_lr_scan()
    ...
    def start_2D_scan(self):
        self._scan_mngr.start_2D_scan()

    # def activate_2D_scan (self):
    #     if (self._running_task==None):
    #         self.curr_pz_volt = self._scan_mngr.minV_lengthscan
