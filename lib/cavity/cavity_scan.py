...
class CavityScanManager ():

	def __init__ (self, adwin, wm_adwin, laser):
		self._adwin = adwin
	...
	def record_time_series (self, nr_samples, wait_cycles =1):
		self.PD_signal, self.timestamps = self._adwin.timeseries_photodiode (nr_steps = nr_samples+30, wait_cycles = wait_cycles)
		self.PD_signal = self.PD_signal[30:-1]
		self.timestamps = self.timestamps[30:-1]


	def start_cavity_tracking (self, ctr_V, v_step, nr_steps, nr_tracks, nr_avg_reps):
		self.PD_signal, self.V, self.timestamps = self._adwin.fast_cavity_piezo_tracking (nr_steps = nr_steps, wait_cycles = 1, center_voltage = ctr_V, voltage_step=v_step, nr_avg_reps =nr_avg_reps, nr_tracking_steps = nr_tracks)
