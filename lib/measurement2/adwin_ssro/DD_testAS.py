...
class Gate(object):
    ...
    def __init__(self,name,Gate_type,**kw):
        ...
        self.source     = kw.pop('mw_source',None)      ### select which mw source to use
...
class DynamicalDecoupling(pulsar_msmt.MBI):
    ...
    def _X_elt(self):
        ...
        if not choose_source:
            ...
            if self.params['electron_transition_used'] == '_p1':
                print 'Transition is p1'
                X = ps.X_pulse(self)
                return X 
            elif self.params['electron_transition_used'] == '_m1':
                print 'Transition is m1'
                X = ps.mw2_X_pulse(self)
                return X
            else:
                print 'Please define the used transition: transition is (%s)' %self.params['electron_transition_used']
    ...
    def determine_decoupling_scheme(self,Gate):
        ...
        if choose_source:
            ...
            if self.params['electron_transition_used'] == '_m1':
                pi_pulse_duration= self.params['mw2_fast_pi_duration']
        else:
            pi_pulse_duration= self.params['fast_pi_duration']
