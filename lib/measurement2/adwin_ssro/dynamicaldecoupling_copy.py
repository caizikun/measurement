...
note (01-06-2015): This file is a version of DynamicalDecoupling that does not refer to pulse_select.
That is, it uses all 'hard-coded' square pulses that were present in the last version that was pulled from github on 27-05-2015.
-- KvB
'''
import numpy as np
...
class DynamicalDecoupling(pulsar_msmt.MBI):
    ...
    def generate_decoupling_sequence_elements(self,Gate):
        ...
        if Gate.scheme == 'single_block':
            ...
            ### Probably not necessary to define initial/final phase if you refer to _pi2_elt() and _mpi2_elt()
            if self.params['Initial_Pulse'] =='-x':
                initial_phase = self.params['X_phase']+180
            else:
                initial_phase = self.params['X_phase']
