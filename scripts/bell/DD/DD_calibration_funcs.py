...
from measurement.scripts.bell.DD import dd_measurements as dd_msmt
reload(dd_msmt)
...
def cal_CORPSE_pi(name, multiplicity = 1):
    m = CORPSE_calibration.CORPSEPiCalibration(name+'M={}'.format(multiplicity))
    funcs.prepare(m)
...
def dd_calibrate_C13_revival(name):
    ...
    m.params_lt3['DD_pi_phases'] = [90,0,90] ## THIS DEFINES THE XYX SEQUENCE
    m.params_lt3['CORPSE_pi2_wait_length']= 1300e-9
    m.params_lt3['first_C_revival'] = 106.579e-6/2. #C revival after CORPSE_pi2_wait_length 
    ...
    m.params_lt3['free_evolution_times'] = 52.4e-6 + np.linspace(-5e-6, 5e-6, pts) #m.params_lt3['first_C_revival'] #
...
def dd_calibrate_T2(name):
    ...
    sweep_array=  m.params_lt3['first_C_revival'] + np.linspace(-1e-6, 1e-6, pts_per_revival)
...
def dd_sweep_LDE_spin_echo_time(name):
   ...
   # m.params_lt3['A_SP_amplitude'] = 0. #use LDE element SP. 
    ...
    # sweep params
    m.params_lt3['dd_spin_echo_times'] = np.linspace(-150e-9, 0e-9, pts)
...
def dd_sweep_LDE_DD_XYX_t_between_pulse(name):
    ...
    # sweep params
    m.params_lt3['extra_ts_between_pulses'] = 400e-9 + np.linspace(-2e-6,2e-6,pts)
...
def run_calibrations(stage):  
    ...
    if stage == 1:
        print 'Cal CORPSE pi'
        cal_CORPSE_pi(name, multiplicity=1)
    if stage == 1.5:
...
if __name__ == '__main__':
    #execfile('d:/measuring/measurement/scripts/lt3_scripts/setup/msmt_par-ms.py')
    #run_calibrations(0)
    ...
    stage 0.5: dark ESR
            --> f_msm1_cntr_lt3 in parameters.py AND msmt_params.py
    ...
    # note: analyze all stages with LT2_calibrations.py (imported as lt3cal)
    """
