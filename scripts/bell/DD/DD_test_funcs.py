...
from measurement.scripts.lt3_scripts.adwin_ssro import espin_funcs as funcs
reload(funcs)
...
def dd_calibrate_C13_revival(name):
    ...
    m.params_lt3['free_evolution_times'] = 53.4e-6 + np.linspace(-2.5e-6, 2.5e-6, pts) #m.params_lt3['first_C_revival'] #
...
def dd_bare_XY4(name):
    ...
    # sweep params
    #free evolutiona time is half the total evolution time!!! from centre to centre of pulses
    m.params_lt3['free_evolution_times'] = m.params_lt3['first_C_revival'] + np.linspace(-1e-6, 1e-6, pts) # #
...
def dd_sweep_LDE_spin_echo_time(name):
    ...
    #free evolutiona time is half the total evolution time!!! from centre to centre of pulses
    m.params_lt3['free_evolution_times'] = np.ones(pts) * m.params_lt3['first_C_revival']
    m.params_lt3['extra_ts_between_pulses'] = np.ones(pts) * 0 
   # m.params_lt3['A_SP_amplitude'] = 0. #use LDE element SP. 
    
    # sweep params
    m.params_lt3['dd_spin_echo_times'] = np.linspace(450e-9, 610e-9, pts)
    
    m.params_lt3['DD_pi_phases'] = [0]
    m.dd_sweep_LDE_spin_echo_time_msmt()

    # for the autoanalysis
    m.params_lt3['sweep_name'] = 'extra dd spin echo time (us)'
...
def dd_sweep_free_ev_time_with_LDE(name):
    ...
    m.params_lt3['dd_spin_echo_times'] = np.ones(pts) * m.params_lt3['dd_spin_echo_time']
...
def dd_sweep_LDE_DD_XY4_t_between_pulse(name):
    ...
    # sweep params
    m.params_lt3['extra_ts_between_pulses'] = np.linspace(-30e-9,80e-9,pts)
...
def dd_sweep_LDE_DD_XY4_pi2_pulse_amp(name):
    ...
    # sweep params  
    m.params_lt3['pi2_pulse_amps'] = np.linspace(-0.15, 0.15, pts) + m.params_lt3['CORPSE_pi2_amp']
...
def dd_sweep_LDE_DD_XY4_spin_echo(name):
    ...
    m.params_lt3['free_evolution_times'] = np.ones(pts) * m.params_lt3['first_C_revival']
    #m.params_lt3['A_SP_amplitude'] = 0. #use LDE element SP. 
    m.params_lt3['extra_ts_between_pulses'] = np.ones(pts) * m.params_lt3['dd_extra_t_between_pi_pulses']
    # sweep params
    
    m.params_lt3['dd_spin_echo_times'] = np.linspace(-40e-9,40e-9,pts) + m.params_lt3['dd_spin_echo_time']
...
def dd_sweep_LDE_DD_XY4_free_evolution_time(name):
    ...
    m.params_lt3['free_evolution_times'] = np.linspace(-1e-6,1e-6,pts) + m.params_lt3['first_C_revival']
...
def dd_sweep_LDE_DD_YXY_free_evolution_time(name):
    ...
    # sweep params
    m.params_lt3['pi2_pulse_phase'] = -90
    m.params_lt3['free_evolution_times'] = np.linspace(-1e-6,1e-6,pts) + m.params_lt3['first_C_revival']
...
def dd_sweep_LDE_DD_XY4_free_evolution_time_t_coh(name):
    ...
    #free evolutiona time is half the total evolution time!!! from centre to centre of pulses
    m.params_lt3['extra_ts_between_pulses'] = np.ones(ppts) * m.params_lt3['dd_extra_t_between_pi_pulses']
    m.params_lt3['dd_spin_echo_times'] = np.ones(ppts) * m.params_lt3['dd_spin_echo_time']
    #m.params_lt3['A_SP_amplitude'] = 0. #use LDE element SP. 
    
    # sweep params
    
    m.params_lt3['free_evolution_times1'] = np.linspace(-1e-6,1e-6,pts) + m.params_lt3['first_C_revival']
    m.params_lt3['free_evolution_times2'] = np.linspace(-1e-6,1e-6,pts) + 2 * m.params_lt3['first_C_revival']
    m.params_lt3['free_evolution_times3'] = np.linspace(-1e-6,1e-6,10) + 3 * m.params_lt3['first_C_revival']
    m.params_lt3['free_evolution_times4'] = np.linspace(-1e-6,1e-6,pts) + 4 * m.params_lt3['first_C_revival']

    m.params_lt3['free_evolution_times'] = np.linspace(-5e-6,5e-6,pts) + 3 * m.params_lt3['first_C_revival']
...
if __name__ == '__main__':
    execfile('d:/measuring/measurement/scripts/lt3_scripts/setup/msmt_params.py')
    #run_calibrations(1)
    #run_calibrations(2)
    #run_calibrations(3) 
    run_calibrations(4) 
