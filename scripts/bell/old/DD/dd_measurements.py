...
from measurement.scripts.bell import parameters as tparams
reload(tparams)
from measurement.scripts.bell import sequence as tseq
reload(tseq)
...
def prepare(m):
    ...
    m.params_lt3 = m2.MeasurementParameters('LT3Parameters')
...
def finish(m, upload=True, debug=False, **kw):
    for key in m.params_lt3.to_dict():
        m.params[key] = m.params_lt3[key]
...
class DynamicalDecoupling(pulsar_msmt.PulsarMeasurement):
    ...
    def load_settings(self):
        ...
        for k in tparams.params_lt3.parameters:
            self.params_lt3[k] = tparams.params_lt3[k]

        self.params['opt_pi_pulses'] = OPT_PI_PULSES
        self.params_lt3['MW_during_LDE'] = 1 if LDE_DO_MW else 0

    def update_definitions(self):
        tseq.pulse_defs_lt3(self)
        self.adwin_lt3_trigger_element = tseq._lt3_adwin_lt3_trigger_elt(self)

    def LDE_element(self, **kw):
        return tseq._lt3_LDE_element(self, **kw)
        
    def first_pi2(self, **kw):
        # kws are init_ms1, CORPSE_pi_shelv_amp, CORPSE_pi2_amp
        return tseq._lt3_first_pi2(self, **kw)

    def second_pi2(self, name, time_offset, **kw):
        # kws are extra_t_before_pi2, CORPSE_pi2_amp
        return tseq._lt3_final_pi2(self, name, time_offset, **kw)
    ...
    def dynamical_decoupling(self, seq, time_offset, **kw):
        ...
        return tseq._lt3_dynamical_decoupling(self, seq, time_offset, **kw)

    def generate_sequence(self, upload = True):
        if upload:
            #qt.pulsar.upload(*self.elements)
            qt.pulsar.program_awg(self.seq,*self.elements)
    ...
    def dd_sweep_free_ev_time_msmt(self):
        ...
        for i in range(self.params['pts']):
            ...
            seq, total_elt_time, elts = self.dynamical_decoupling(self.seq, 
                ...
                begin_offset_time = 0.,
                free_evolution_time = self.params_lt3['free_evolution_times'][i],
            ...
            self.seq.append(name = 'sync_elt-{}'.format(i), 
                wfname = self.adwin_lt3_trigger_element.name)
        ...
        self.elements.append(self.adwin_lt3_trigger_element)
    ...
    def dd_sweep_LDE_spin_echo_time_msmt(self):
        ...
        LDE_elt = self.LDE_element(name = 'LDE_element',
                pi2_pulse_phase = self.params_lt3['pi2_pulse_phase'])
        ...
        for i in range(self.params['pts']):
            ...
            self.seq, total_elt_time, elts = self.dynamical_decoupling(self.seq, 
                ...
                begin_offset_time = self.params_lt3['dd_spin_echo_times'][i],
                free_evolution_time = self.params_lt3['free_evolution_times'][i], 
                extra_t_between_pulses = self.params_lt3['extra_ts_between_pulses'][i],
                use_delay_reps = self.params['dd_use_delay_reps'],
                name = i)

            second_pi2_elt = self.second_pi2(name = i, 
                time_offset = LDE_elt.length() + total_elt_time, 
                CORPSE_pi2_phase = self.params_lt3['pi2_pulse_phase'])
...
def dd_sweep_free_ev_time(name):
    ...
    m.params_lt3['free_evolution_times'] = np.linspace(-15e-6, 15e-6, pts) + \
...
def dd_sweep_free_ev_time_with_LDE(name):
    ...
    params_lt3['extra_ts_between_pulses'] = np.ones(pts) * 0
...
def dd_sweep_LDE_spin_echo_time(name):
    ...
    # sweep params
    m.params_lt3['dd_spin_echo_times'] = np.linspace(-300e-9, 500e-9, pts)
