...
After that a last gate is performed to intialize C1 via SWAP

Gate scheme, assuming decent C1 control:
|C1 MBI| - |wait time: beating freq*2 = pi| - |M C1 X|(require X) - |C1 SWAP part|
'''
...
if __name__ == '__main__':
    ...
    # detuning = 1e3
    el_RO = ['positive','negative']
    for e in el_RO:
        breakst = show_stopper()
        if breakst:
            break
        NuclearInitializationWithRemoteInitialization('CarbonRemoteInitialization_' + e + '_C' + str(carbon)+'_'+ EL_TRANS[-2:] ,
            carbon_nr= carbon,
