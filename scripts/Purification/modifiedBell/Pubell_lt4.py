...
import Pubell
reload(Pubell)
import Pusequence as bseq
reload(bseq)
import Pujoint_params
reload(Pujoint_params)
import Puparams_lt4
reload(Puparams_lt4)

class Bell_lt4(Pubell.Bell):
    mprefix = 'Bell_lt4'
    adwin_process = 'bell_lt4'

    def __init__(self, name):
        Pubell.Bell.__init__(self,name)
        for k in Pujoint_params.joint_params:
            self.joint_params[k] = Pujoint_params.joint_params[k]
        for k in Puparams_lt4.params_lt4:
            self.params[k] = Puparams_lt4.params_lt4[k]
    ...
    def print_measurement_progress(self):
        if self.PQ_ins.name == 'HH_400':
            print 'Hello, I am lt4 and I use the HydraHarp for my measurement'
            ...
            self.physical_adwin.Set_Par(51, int(tail_cts))
            self.physical_adwin.Set_Par(52, int(pulse_cts))
            # self.physical_adwin.Set_Par(53, int(sp_lt4_reflection))
            # self.physical_adwin.Set_Par(54, int(sp_lt3_reflection))

            self.adwin_ins_lt4.Set_Par(78, int(self.entanglement_markers))

        else:
            Pubell.print_measurement_progress()

    def measurement_process_running(self):
        return self.lt4_helper.get_is_running() and Pubell.Bell.measurement_process_running(self)

    def finish(self):
        Pubell.Bell.finish(self)
...
def bell_lt4(name, 
    ...
    if not(sequence_only):
        if measure_lt3:
            ...
            m.lt3_helper.set_script_path(r'Y:/measurement/scripts/Purifcation/modifiedBell/Pubell_lt3.py')
...
def measureXX(name):
    ...
    m.params['MW_RND_amp_Q']     = 0#-m.params['MW_pi2_amp'] 
...
def measureZZ(name):
    name='MeasZZ_'+name
    m = Bell_lt4(name)
    m.params['MW_RND_amp_I']     = 0 #m.params['MW_pi_amp'] 
...
def SP_PSB_RandomMW(name):
    ...
    m.params['MW_RND_duration_I']= m.params['MW_pi_duration'] 
    m.params['MW_RND_amp_Q']     = 0#m.params['MW_pi_amp']
    m.params['MW_RND_duration_Q']= m.params['MW_pi_duration']
