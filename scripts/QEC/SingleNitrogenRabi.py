...
def CarbonRabiWithDirectRF(name,
    ...
    m = DD.NitrogenRabiWithDirectRF(name)
    funcs.prepare(m)
...
if __name__ == '__main__':
    ...
    DoRabi = True
    Sweep_C_RO_phase = False
    CarbonRabiWithDirectRF(SAMPLE + 'Rabi_C'+str(carbon_nr)+'_el1_positive_' +str(i)+'run', carbon_nr = carbon_nr, el_RO= 'positive', el_after_init=el_after_init, DoRabi=DoRabi, Sweep_C_RO_phase = Sweep_C_RO_phase, RF_generation_method = 'AWG', debug=True)
