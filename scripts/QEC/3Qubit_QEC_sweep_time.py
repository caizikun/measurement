...
def MBE(name, carbon_list   = [1,5,2],               
    ...
    m.params['wait_in_msm1']  = wait_in_msm1

    # m.params['free_evolution_time_1'] = np.array(evolution_time_list)/2.
    m.params['free_evolution_time_1'] = np.ones(len(evolution_time_list))*50e-3 
    m.params['free_evolution_time_2'] = np.array(evolution_time_list)

    # m.params['C1_freq_0'] +=  300  
    # m.params['C1_freq_1'] +=  300

    phase_error                       = np.zeros(len(m.params['free_evolution_time_1']))
    # phase_error                   = error_sign * 2*np.arcsin(np.sqrt(error_probability_list))*180./np.pi
    
    if error_on_qubit ==1: # carbon 1
        Qe                            = [0,0,1]
    elif error_on_qubit ==2:# carbon 5
        Qe                            = [1,0,0]
    elif error_on_qubit ==3:# carbon 2
        Qe                            = [0,1,0]
...
if __name__ == '__main__':
    cnt = -1
    evo_list = {}
    evo_list_total = np.r_[20e-6,np.linspace(30e-3,70e-3,21)]
    evo_list_total = [50e-3]#[20e-6,50e-3]

    # evo_list_total = [0,40e-3,100e-3]#[0,10e-3,16e-3,20e-3]#[0,30e-3,40e-3,60e-3]
    ...
    evo_list['5'] = evo_list_total[15:18]
    evo_list['6'] = evo_list_total[18:21]
    evo_list['7'] = evo_list_total[21:22]

    for syn_round in [1]:#,0,2,3]:
        ...
        for state in ['mZ']:#,'mZ']:
            ...
            for RO in [0]:#RO_list:
                ...
                for k in range(1):
                    ...
                    t_list = evo_list_total#evo_list[str(k)]
                    print '-----------------------------------'            
                    print 'press q to stop measurement cleanly'
                    print '-----------------------------------'
                    qt.msleep(2)
                    ...
                    if syn_round == 0:
                        ...
                        MBE(SAMPLE + '11_sweep_time_positive_RO'+str(RO)+'_k'+str(k)+'_'+logic_state+'_ms1',RO_C = RO, 
                            logic_state = logic_state,el_RO = 'positive', 
                            error_on_qubit = 'all',
                            evolution_time_list= t_list,
                            wait_in_msm1 = True,
                        ...
                        MBE(SAMPLE + '11_sweep_time_negative_RO'+str(RO)+'_k'+str(k)+'_'+logic_state+'_ms1',RO_C = RO, 
                            logic_state = logic_state,el_RO = 'negative', 
                            error_on_qubit = 'all',
                            evolution_time_list= t_list,
                            wait_in_msm1 = True,
                        ...
                        MBE(SAMPLE + '11_sweep_time_positive_RO'+str(RO)+'_k'+str(k)+'_'+logic_state+'_ms0',RO_C = RO, 
                            logic_state = logic_state,el_RO = 'positive', 
                            error_on_qubit = 'all',
                            evolution_time_list= t_list,
                            wait_in_msm1 = False,
                        ...
                        MBE(SAMPLE + '11_sweep_time_negative_RO'+str(RO)+'_k'+str(k)+'_'+logic_state+'_ms0',RO_C = RO, 
                            logic_state = logic_state,el_RO = 'negative', 
                            error_on_qubit = 'all',
                            evolution_time_list= t_list,
                            wait_in_msm1 = False,
