...
temperature_list.append(temp_0)
j = 0
delta_f0_check = 0
...
for i in range (3000):
    ...
    if __name__ == '__main__':
        ...
        optimization_target = 6     # Target difference frequency in kHz , first (03.08) was 10 then 7 then 10 again
        ...
        temperature = (temperature_sensor.get_readlastval()-100)/0.385 
        temperature_list.append(temperature)

        delta_temperature = temperature_list[i+1]-temperature_list[j]
        delta_f_temperature = 1133*delta_temperature   #in KHz


        if abs(delta_f_temperature-delta_f0_check) > 8:

            delta_move = 1e-3*(-delta_f_temperature+delta_f0_check)/100
            j=i
            ...
            # Measure B-field 
            darkesr('magnet_optimize_after_tcorrct', range_MHz=coarse_range, pts=coarse_pts, reps=coarse_reps, 
                    power= coarse_amplitude, MW_power = coarse_MW_power, pulse_length=coarse_pulse_length)
            ...
            # temperature = (temperature_sensor.get_readlastval()-100)/0.385 # better to be here
            # temperature_list.append(temperature)

            # delta_temp = temperature_list[i+1]-temperature_list[j]
