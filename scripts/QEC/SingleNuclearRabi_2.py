...
def CarbonRabiWithDirectRF(name,
    ...
    if DoRabi: 
        m.params['RF_pulse_durations'] = 10e-6 + np.linspace(0e-6,990e-6,10)
        m.params['pts'] = len(m.params['RF_pulse_durations'])
        m.params['RF_pulse_frqs'] = np.ones(m.params['pts']) * m.params['C' + str(carbon_nr) + '_freq_1_m' + el_after_init] #+ 30e3
        m.params['sweep_name'] = 'RF_pulse_length (us)'
        m.params['sweep_pts']  =  m.params['RF_pulse_durations'] / 1e-6 
    else: 
        centerfreq = m.params['C' + str(carbon_nr) + '_freq_1_m' + el_after_init]
        m.params['RF_pulse_frqs'] = np.linspace(centerfreq-2e3,centerfreq+2e3,15)
        m.params['pts'] = len(m.params['RF_pulse_frqs'])
        m.params['RF_pulse_durations'] = np.ones(m.params['pts']) * 780e-6
...
if __name__ == '__main__':
    ...
    i = 0
    el_after_init = '1'
    CarbonRabiWithDirectRF(SAMPLE + 'Rabi_C'+str(carbon_nr)+'_el1_positive_' +str(i)+'run', carbon_nr = carbon_nr, el_RO= 'positive', el_after_init=el_after_init, DoRabi=False, debug=False)
