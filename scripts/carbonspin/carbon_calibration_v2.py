...
AS Added flexibility in chosing the different eigenstates of the electron'''
    ...
    if not debug:
        ...
        for c in carbons:
            ...
            if f_ms_m1:
                ...
                search_string = 'C'+str(c)+'_freq_1_m1'
                data = write_to_file_subroutine(data,search_string)

            if f_ms_p1:

                search_string = 'C'+str(c)+'_freq_1_p1'
                data = write_to_file_subroutine(data,search_string)

            if self_phase or cross_phase:

                search_string = 'C'+str(c)+'_Ren_extra_phase_correction_list'+qt.exp_params['samples']['111_1_sil18']['electron_transition']
                data = write_to_file_subroutine(data,search_string)
        ...
        f.writelines(data)
        f.close()
        print 'All measured values updated!'
...
if f_ms_m1 and n == 1:
    ...
    for c in carbons:
        ...
        if n == 1:
            ...
            NuclearRamseyWithInitialization_cal(SAMPLE_CFG+'_msm1_freq_C'+str(c), carbon_nr= c, detuning = detuning, el_state = 1, fid_transition = '_m1')
            # fit
            ...
            #update
            qt.exp_params['samples'][SAMPLE]['C'+str(c)+'_freq_1'+'_m1'] += -f0 + detuning
...
if f_ms_p1 and n == 1:
    ...
    for c in carbons:
        ...
        if n == 1:
            ...
            NuclearRamseyWithInitialization_cal(SAMPLE_CFG+'_msm1_freq_C'+str(c), carbon_nr= c, detuning = detuning, el_state = 1,fid_transition = '_p1'))
            # fit
            f0, uf0 = cr.Carbon_Ramsey(timestamp=None, 
                          offset = 0.5, amplitude = 0.3, x0=0, decay_constant = 1e5, exponent = 2, 
                          frequency = detuning, phase =0, 
                          ...
                          title = 'msm1_freq_C'+str(c))
            #update
            qt.exp_params['samples'][SAMPLE]['C'+str(c)+'_freq_1'+'_p1'] += -f0 + detuning
...
if n == 1 and self_phase_calibration:
    for c in carbons:
        if qt.exp_params['samples']['111_1_sil18']['multiple_source']:
            qt.exp_params['samples']['111_1_sil18']['electron_transition']=qt.exp_params['samples']['111_1_sil18']['C'+str(c)+'_dec_trans']
            electron_transition_string=qt.exp_params['samples']['111_1_sil18']['C'+str(c)+'_dec_trans']
        if n == 1:
...
if cross_phase_calibration and n ==1 and len(carbons)>1:
    ...
    for c in carbons:
        ...
        for c_cross in carbons_cross:
            if qt.exp_params['samples']['111_1_sil18']['multiple_source']==True:
                qt.exp_params['samples']['111_1_sil18']['electron_transition']=qt.exp_params['samples']['111_1_sil18']['C'+str(c_cross)+'_dec_trans']
                electron_transition_string=qt.exp_params['samples']['111_1_sil18']['C'+str(c)+'_dec_trans']
...
if f_ms_m1:
    ...
    for c in carbons:
        print 'Carbon '+str(c)
        print qt.exp_params['samples'][SAMPLE]['C'+str(c)+'_freq_1_m1']
    print
    print '#########################'
...
if f_ms_p1:
    ...
    for c in carbons:
        print 'Carbon '+str(c)
        print qt.exp_params['samples'][SAMPLE]['C'+str(c)+'_freq_1_p1']
    print
    print '#########################'
