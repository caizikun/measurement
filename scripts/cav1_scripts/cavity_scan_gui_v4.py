...
from scan_panel import Ui_Form as Ui_Scan
...
class ScanGUI(QtGui.QMainWindow):
    def __init__(self, scan_manager):
        ...
        self.ui.sb_nr_steps_pzscan.setRange(1, 999)
        ...
        self.ui.sb_nr_steps_timeseries.setRange(1,99999)
        self.ui.sb_wait_cycles.setRange(1,999)
    ...
    def start_timeseries(self):
        if (self._running_task==None):
            self._running_task = 'timeseries'
 
    def stop_timeseries(self):
        if (self._running_task == 'timeseries'):
            self._running_task = None
    ...
    def run_new_pzscan(self):
        ...
        self._scan_mngr.piezo_scan(avg_nr_samples=self._scan_mngr.averaging_samples)
    ...
    def run_new_fine_laser_scan(self):
        ...
        self._scan_mngr.set_scan_params (v_min=self._scan_mngr.minV_finelaser, v_max=self._scan_mngr.maxV_finelaser, nr_points=self._scan_mngr.nr_steps_lr_scan)
    ...
    def run_new_lr_laser_scan (self):
        ...
        self._scan_mngr.set_scan_params (v_min=-3, v_max=3, nr_points=self._scan_mngr.nr_steps_finelaser) #real fine-scan
        ...
        self.curr_l = self.curr_l + self.coarse_wavelength_step
        
        if (self.curr_l>self._scan_mngr.max_lambda):
            self._running_task = None
            if self._scan_mngr.autosave:
                self.save()
        else:
