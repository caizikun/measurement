...
def generate_LDE_elt(msmt,Gate, **kw):
    ...
    if msmt.params['MW_during_LDE'] == 1: # and not ('LDE2' in Gate.name):
        ...
        # MW pi pulse
        if msmt.params['do_final_mw_LDE'] == 1:
            ...
            e.add(pulse.cp(Gate.mw_pi2,
                ...
                start           = msmt.params['LDE_element_length']-msmt.params['initial_delay']-2.5e-6,
                refpulse        = 'initial_delay',
                refpoint        = 'end',
                refpoint_new    = 'center',
                name            = 'MW_RO_rotation')
            ...
            e.add(Gate.mw_X,
                start           = msmt.params['LDE_element_length']-msmt.params['initial_delay']-(msmt.params['LDE_decouple_time']-msmt.params['average_repump_time']),
    ...
    ### we still need MW pulses (with zero amplitude) as a reference for the first optical pi pulse.
        ...
        # MW pi pulse
        e.add(pulse.cp(Gate.mw_second_pulse,amplitude=0),
            start           = msmt.params['LDE_element_length']-msmt.params['initial_delay']-2.5e-6-(msmt.params['LDE_decouple_time']),
            refpulse        = 'initial_delay',#'MW_Theta',
            refpoint        = 'end',#'end',
            refpoint_new    = 'center',#'end',
            name            = 'MW_pi')
    ...
    #4 opt. pi pulses
    # print 'Nr of opt pi pulses', msmt.params['opt_pi_pulses']
    ...
    if not ((msmt.params['LDE_1_is_init'] > 0) and 'LDE1' in Gate.name):
        ...
        for i in range(msmt.params['opt_pi_pulses']):
            ...
            start = msmt.params['opt_pulse_separation'] if i > 0 else msmt.params['MW_opt_puls1_separation']
        ...
        if msmt.params['PLU_during_LDE'] == 1 :
            ...
            if msmt.params['opt_pi_pulses'] > 1:
                e.add(Gate.plu_gate, 
                    name = 'plu gate 2',
    ...
    if Gate.is_final:
        ...
        ## otherwise put the pulse at the end of the LDE sequence
            ...
            e.add(Gate.adwin_trigger_pulse,
                    start = msmt.params['LDE_element_length'] - 2.6e-6,
                    refpulse = 'initial_delay')

   
    # Gate.reps = msmt.params['LDE_attempts_before_CR']
    Gate.elements = [e]
    Gate.elements_duration = msmt.params['LDE_element_length']
    ...
    # if e_len != msmt.params['LDE_element_length']:
    #     raise Exception('LDE element "{}" has length {:.6e}, but specified length was {:.6e}. granularity issue?'.format(e.name, e_len, msmt.params['LDE_element_length']))
