...
class GreenRO_T1(Green_RO):
    """
    Simply prepares electron in ms=0, waits and reads out with green laser
    """
    # def generate_sequence(self, upload = True):
    #     """
    #     Pulses = 
    #     - initialization pulse
    #     - short waiting pulse (100 ns)
    #     - trigger to Adwin/QuTau to start measuring
    #     - wait element of variable duration (= sweeped parameter)
    #       NOTE: a waiting element of length 'wait_time_repeat_element' is repeated if duration of wait element > wait_time_repeat_element
    #       If not, then NO WAIT ELEMENT is programmed in the AWG!
    #     - 2 syncs to QuTau to stop measuring (2 required to reduce probability of sync being missed) 
    #     """
    ...
    #     ### Create list of elements from the basic pulses ###
    #     list_of_elements = []

    #     # Add spin polarization pulse at beginning to initialize system
    #     init = element.Element('initialize', pulsar = qt.pulsar)
    #     init.add(pulse.cp(sq_AOMpulse, length = 1e-6, amplitude = 0), name = 'wait')
    #     init.add(sq_AOMpulse,name='init',refpulse='wait')

    #     list_of_elements.append(init)

    #     # Add wait time element
    ...
    #     # Add readout
    #     e = element.Element('readout', pulsar = qt.pulsar, global_time = True)
    #     # e.add(sq_AOMpulse,name='GreenLight',refpulse='T1_wait_time',refpoint='end')
    #     e.add(sq_AOMpulse,name='GreenLight',refpoint='end')
    #     # e.add(sync,name='Sync',refpulse='ElectronT1_wait_time',refpoint='end')
    #     e.add(sync,name='Sync',refpoint='end')
    #     e.add(pulse.cp(T,length=self.params['time_between_syncs']),name='wait3',refpulse='Sync',refpoint='end')
    #     e.add(sync,name='Sync2',refpulse='wait3',refpoint='end')
    ...
    #     # End delay: inserted at end of sequence such that all outputs of the AWG stay low.
    #     end = element.Element('ending delay', pulsar = qt.pulsar)
    #     end.add(pulse.cp(sq_AOMpulse, length = 1e-6, amplitude = 0), name = 'delay')
        
    #     list_of_elements.append(end)


    #     ### Create sequences from elements / waveforms ###
    ...
    #     first_element = -1 # Counter for first element

    #     for k, t in enumerate(self.params['wait_times']):
    #         # Waiting time 

    #         if self.params['wait_times'][k]/self.params['wait_time_repeat_element'] !=0:
    #             seq.append(name = "T1_wait_time_%d" % k, wfname = 'T1_wait_time', 
    #                 trigger_wait = False, repetitions = self.params['wait_times'][k]/self.params['wait_time_repeat_element'])
    #             if first_element == -1:
    #                 first_dp_element = "T1_wait_time_%d" % k
    #                 first_element += 1
            
    #         # Readout
    #         seq.append(name = 'readout_%d' % k, wfname = 'readout', trigger_wait = False, jump_target = init.name )

    #     seq.append(name = end.name, wfname = end.name, goto_target = first_dp_element, jump_target = init.name)

    #     # for k,t in enumerate(self.params['wait_times']):
    #     #     e=element.Element('T1_length-%d' % k,pulsar=qt.pulsar)
    #     #     if k==0:
    #     #         first_dp_element=e.name

    #     #     e.add(T(length=100e-9),name='wait') # Changed 1500 --> 100
    #     #     e.add(adwin_trigger_pulse,name='adwin_trigger',refpulse='wait',refpoint='start')

    #     #     seq.append(name = e.name, wfname = 'Initialization', trigger_wait = False)

    #     #     if self.params['wait_times'][k]/self.params['wait_time_repeat_element'] !=0:
    #     #         e.add(T,name='ElectronT1_wait_time_%d' % k,refpulse='adwin_trigger',refpoint='end', repetitions=t/self.params['wait_time_repeat_element'])
    #     #     else:
    #     #         e.add(T(length = t), name='ElectronT1_wait_time_%d' % k,refpulse='adwin_trigger',refpoint='end')
    #     #     e.add(sq_AOMpulse,name='GreenLight',refpulse='ElectronT1_wait_time_%d' % k,refpoint='end')
    #     #     e.add(sync,name='Sync',refpulse='ElectronT1_wait_time_%d' % k,refpoint='end')
    #     #     e.add(pulse.cp(T,length=self.params['time_between_syncs']),name='wait3',refpulse='Sync',refpoint='end')
    #     #     e.add(sync,name='Sync2',refpulse='wait3',refpoint='end')
    ...
    #             qt.pulsar.program_sequence(seq,loop=False)
    #         else:
    #             qt.pulsar.program_awg(seq,*list_of_elements,loop=False)
...
def Rabi(name):
    ...
    m.params['nr_of_cycles'] = 35                       # nr of times we read out the qutau
                                                        # nr of reps per datapoint is nr_of_cycles*repetitions/pts/2 (2 comes from 2 syncs per RO)
    #About the sweep
    m.params['sweep_name'] = 'Pulse Length [us]'
    m.params['MW_pulse_durations'] = np.linspace(0,2.2,m.params['pts'])*1e-6
    m.params['sweep_pts']  = m.params['MW_pulse_durations']*1e6
    

    # About the MW's
    m.params['ssbmod_amplitude']= 0.5 / 3. / 4. / 9.
...
def Dark_ESR(name):
    ...
    m.params['pts'] = 31
    m.params['repetitions'] =  0.5e6/m.params['pts']/2  #nr of repetitions per cycle
    m.params['nr_of_cycles'] = 500                       # nr of times we read out the qutau
                                                        # nr of reps per datapoint is nr_of_cycles*repetitions/pts/2 (2 comes from 2 syncs per RO)
    #About the sweep
    m.params['sweep_name'] = 'Frequency [GHz]'
    f_range = 4.5e6
    m.params['MW_pulse_duration'] = 1.083e-6
    f_center = m.params['MW_modulation_frequency']
    m.params['MW_modulation_frequencies'] = np.linspace(f_center-f_range,f_center+f_range,m.params['pts'])
    m.params['sweep_pts']  = m.params['MW_modulation_frequencies']+m.params['mw_frq']
...
def Ramsey(name):
    ...
    m.params['pts'] = 30 # 41
    m.params['repetitions'] =  0.5e6/m.params['pts']/2  #nr of repetitions per cycle
    m.params['nr_of_cycles'] = 25 #75                       # nr of times we read out the qutau
                                                        # nr of reps per datapoint is nr_of_cycles*repetitions/pts/2 (2 comes from 2 syncs per RO)
