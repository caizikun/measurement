        ...
        for j,gv in enumerate(np.linspace(self.gate_start_voltage, self.gate_stop_voltage, self.gate_pts)):
            ...
            if self.plot_strain_lines:
                ...
                try:
                    ...
                    lx,ly=nvlevels.get_transitions_ExEy_plottable(Ex_line,Ey_line,max(d.get_data()[:,2]),B_field=[0.,0.,self.Bz])
...
class LabjackAdwinLaserScan(LaserFrequencyScan):
    def __init__(self, name,labjack_dac_nr):
        ...
        self.mw = qt.instruments['SGS100A_2']
        self.mw_2 = qt.instruments['SGS100A_1']
        ...
        ### when using adwin
        self.set_laser_voltage = lambda x: self.adwin.set_dac_voltage(('newfocus_frq',x))
        self.get_laser_voltage = lambda: self.adwin.get_dac_voltage('newfocus_frq')
...
def red_laser_scan(name):
    labjack_dac_nr = 4 # 2 is coarse and 3 is fine for NF. not used in M1 at the moment
    m = RedLaserScan(name,labjack_dac_nr)

    m.Bz = 404

    # Hardware setup
    m.wm_channel = 1 #1 for newfocus, #3 for DLPRO (because these are ADWIN Fpra numbers x+40, not the actual WM chanels) 
    m.frq_offset = 0
    m.frq_factor = 1
    m.counter_channel = 0
    ...
    try:
        m.prepare_scan()
        m.run_scan(stabilizing_time=0.01)
    finally:
        m.finish_scan()
