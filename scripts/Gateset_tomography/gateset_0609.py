...
def gateset_NoTriggers(name, debug = False, fid_1 = ['e'], fid_2 = ['e'], sequence_type = 'specific_germ_position', germ = ['e'], germ_position = [1,3], N_decoupling = [4], run_numbers=[1,2,3]):
    ...
    #CAN BE FOUND HERE
    sequence_length = max(len(fid_1), len(fid_2), len(germ))*(m.params['Hermite_pi2_length']+t_bw_germs)+t_bw_germs
...
def individual_awg_write_ro(filename, awg_size = 50):
    ...
    # for i in range(1):
        ...
        gateset_NoTriggers(name, debug = True, fid_1 = fid_1, fid_2 = fid_2, sequence_type = 'all', germ = germ, N_decoupling = N_dec, run_numbers=run)
