...
' compared to 'normal' ssro, it includes the possibility to read-out the population in ms=+1, 
' by bringing it to ms=0 with a pi pulse and reading out again ms=0
'
' protocol:
' mode  0:  CR check
...
DIM DATA_25[max_repetitions] AS LONG  ' SSRO counts spin readout (global, including both ms=0 and ms=+1)
DIM DATA_27[max_repetitions] AS LONG  ' SSRO counts in ms=+1
    ...
    SELECTCASE mode
      ...
      CASE 4    ' spin readout
        ...
        IF (timer = SSRO_duration) THEN
          ...
          if (counts > 0) then
            data_25[sweep_index]=1
            if (RO_p1>0) then
              data_27[sweep_index]=1
            endif
          endif
