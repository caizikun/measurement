  ...
  ELSE
    ...
    SELECTCASE mode
      ...
      CASE 11
        ...
        p_real[1+k_sweep]=0.5*p_real[1+k_sweep]+0.25*(sin1*(p0i_min - p0_imag [1+k_sweep+t_n]))  
        mode = 12
        
      CASE 12
        p_imag [1+k_sweep] = p0_imag[1+k_sweep] + 0.25*(cos1*(p0i_min + p0_imag [1+k_sweep+t_n]))
        mode = 13
        
      CASE 13
        p_imag[1+k_sweep]=0.5*p_imag[1+k_sweep]-0.25*(sin1*(p0_real [1+abs(k_sweep-t_n)] - p0_real [1+k_sweep+t_n])) 
        timer=-1
        inc(k_sweep)
      ...
      CASE 16 'Bayesian update of ms=1 (0=click) for phase estimation (distributed in 4 modes)  
        p_real [1+k_sweep] = 0.5*p0_real[1+k_sweep] + 0.25*(cos0*(p0_real [1+abs(k_sweep-t_n)] + p0_real [1+k_sweep+t_n])) 
        mode = 17
      ...
      CASE 17
        ...
        p_real[1+k_sweep]=p_real[1+k_sweep]+0.25*(sin0*(p0i_min - p0_imag [1+k_sweep+t_n]))  
        mode = 18
        
      CASE 18
        p_imag [1+k_sweep] = 0.5*p0_imag[1+k_sweep] + 0.25*(cos0*(p0i_min + p0_imag [1+k_sweep+t_n]))
        mode = 19
        
      CASE 19
        p_imag[1+k_sweep]=p_imag[1+k_sweep]-0.25*(sin0*(p0_real [1+abs(k_sweep-t_n)] - p0_real [1+k_sweep+t_n])) 
        timer=-1
        inc (k_sweep)
        if (k_sweep>2^N+1) then
          inc(update_zero)
