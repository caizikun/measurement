  ...
  ELSE
    ...
    SELECTCASE mode
        ...
        IF (timer = SSRO_duration) THEN
          ...
          ELSE
            ...
            IF (curr_m_step < M) THEN
              mode = 4 ' either go CR are go SP for every M
              timer = -1
              inc(curr_m_step)
            ELSE
              do_event_jump = 1
      ...
      CASE 9 'Bayesian update rule for phase estimation
        ...
        p_real [k] = 0'0.5*p0_real[k] + 0.25*(COS(c_n)*(p0_real [k-t_n] + p0_real [k+t_n]) - SIN(c_n)*(p0_imag [k-t_n] - p0_imag [k+t_n])) 
        p_imag [k] = 0'0.5*p0_imag[k] + 0.25*(COS(c_n)*(p0_imag [k-t_n] + p0_imag [k+t_n]) + SIN(c_n)*(p0_real [k-t_n] - p0_real [k+t_n])) 
        k_for = k_for + 1
