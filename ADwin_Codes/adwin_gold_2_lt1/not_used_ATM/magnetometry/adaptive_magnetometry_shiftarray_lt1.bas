...
#DEFINE max_stat             10
#DEFINE max_prob_array       16400 'OK up to N=12 measurements
...
DIM aux1[max_prob_array] AS FLOAT
DIM aux2[max_prob_array] AS FLOAT
...
DIM N, t_n, curr_adptv_phase, dig_phase, curr_n, rep_index, curr_msmnt_result, curr_m, M, M_count AS LONG
DIM maj_ssro, curr_maj, maj_thr, maj_reps AS LONG
DIM do_calculate_phase, fpga_phase AS LONG
DIM pi, th, real, imag AS FLOAT
DIM timer1, timer2 AS LONG
DIM G, F, renorm_ssro_M, nr_ones, nr_zeros, update_zero, update_one AS LONG
DIM fid0, fid1, c0, c1, cos0, cos1, sin0, sin1 AS FLOAT
DIM max_k, zero_pos, len_array as LONG
  ...
  ELSE
    ...
    SELECTCASE mode
      ...
      CASE 2 'estimate optimal phase according to Cappellaro protocol
        ...
        curr_adptv_phase = round(0.5*th*(180/pi))
      ...
      CASE 3    'set phase to fpga
        ...
        fpga_phase = mod(curr_adptv_phase + DATA_27[curr_n], 360)
        ...
        DATA_33 [sweep_index] = curr_adptv_phase
      ...
      CASE 7    'set Bayesian update parameters --- renormalize ssro
        ...
        c1 = -1*pi-curr_adptv_phase*pi/180
        c0 = 0*pi-curr_adptv_phase*pi/180
        cos0 = COS(c0)
        sin0  = SIN(c0)
        cos1 = COS(c1)
        sin1  = SIN(c1)  
        len_array = max_k - 2*t_n
      ...
      CASE 8 'check for-loop condition (ones)
        k_copy = 1
        DATA_35[sweep_index] = update_zero+1000*update_one
      ...
      CASE 10 'Bayesian update of ms=0 (1=click) for phase estimation (real part)          
        MemCpy (p0_real[1+2*t_n], aux1[1], len_array)
        MemCpy (p0_real[1], aux2[1], len_array)
        MemCpy (p0_real[1+t_n], aux0[1], len_array)
        aux0 = 0.5*aux0 + 0.25*cos1*(aux1+aux2)
        
      CASE 11
        p_real[1+t_n]=p_real[1+t_n]-0.25*(sin1*(p0_imag [1] - p0_imag [1+2*t_n]))  
        mode = 12
        
      CASE 12
        p_imag [1+t_n] = 0.5*p0_imag[1+t_n] + 0.25*(cos1*(p0_imag [1] + p0_imag [1+2*t_n]))
        mode = 13
        
      CASE 13
        p_imag[1+t_n]=p_imag[1+t_n]+0.25*(sin1*(p0_real [1] - p0_real [1+2*t_n])) 
        timer=-1
        inc(update_one)
        k_copy = 1
        mode = 8
 
      CASE 14 'check for-loop condition (ones)
        k_copy = 1
        DATA_34[sweep_index] = update_zero+1000*update_one
      ...
      CASE 16 'Bayesian update of ms=1 (0=click) for phase estimation (distributed in 4 modes)         
        p_real [1+t_n] = 0.5*p0_real[1+t_n] + 0.25*(cos0*(p0_real [1] + p0_real [1+2*t_n])) 
        mode = 17
        
      CASE 17
        p_real[1+t_n]=p_real[1+t_n]-0.25*(sin0*(p0_imag [1] - p0_imag [1+2*t_n]))  
        mode = 18
        
      CASE 18
        p_imag [1+t_n] = 0.5*p0_imag[1+t_n] + 0.25*(cos0*(p0_imag [1] + p0_imag [1+2*t_n]))
        mode = 19
        
      CASE 19
        p_imag[1+t_n]=p_imag[1+t_n]+0.25*(sin0*(p0_real [1] - p0_real [1+2*t_n])) 
        timer=-1
        inc(update_zero)
        k_copy = 1
        mode = 14
