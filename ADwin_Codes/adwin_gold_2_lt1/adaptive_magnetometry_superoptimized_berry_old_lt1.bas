...
'Compared to previous code, we sotre the probability arrays ion a smarter way, storing only the elements we know
...
DIM DATA_33[5000] AS FLOAT            ' array to save optimal Cappellaro phase picked by adwin
...
DIM G, F, m_sweep_index,do_add_phase,added_phase AS LONG
DIM fid0, fid1, c0, c1, cos0, cos1, sin0, sin1, p0i_min, T2 AS FLOAT
Dim var_M, ext_m_msmnt_result,data_len as LONG
...
LowINIT:  
  ...
  if (do_add_phase>0) then
    data_len=N*G + N*F*(N-1)/2
  else
    data_len=repetitions*N   
  endif
  ...
  ELSE
    ...
    SELECTCASE mode
      ...
      CASE 3    'set phase to fpga
        ...
        added_phase=curr_m*180/(G + F*(curr_n-1))
        r_ampl = 0.5*(fid1-fid0)*exp(-(t_n/T2)^2)
        
        fpga_phase = mod(do_adaptive*curr_adptv_phase_deg + DATA_27[curr_n]+added_phase, 360)
        ...
        DATA_33 [sweep_index] = do_adaptive*curr_adptv_phase_deg
      ...
      CASE 7    'electron spin readout
        ...
        IF (timer = SSRO_duration) THEN
          ...
          ELSE
            ...
            curr_maj = 1
            inc(m_sweep_index)
            DATA_25[m_sweep_index] = curr_msmnt_result
      ...
      CASE 17 'possibly save p_k
        if ((save_pk_n > 0))then
          if ((curr_n=save_pk_n) and (curr_m=save_pk_m)) then 'output probability array for debugging
            MemCpy (p_real[1], DATA_38[1], k_range)
            MemCpy (p_imag[1], DATA_39[1], k_range)                  
          endif        
        endif
