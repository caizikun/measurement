    ...
    SELECTCASE mode
      ...
      CASE 1 'SP-E // go to MBI
        ...
        ELSE
          IF (timer = SP_E_duration) THEN
            CNT_ENABLE(0)                                       ' THT: is this nescessary?
            DAC( E_laser_DAC_channel, 3277*E_off_voltage+32768) ' turn off Ex laser
      ...
      CASE 2  'Start Nitrogen MBI seq and wait for trigger // go to MBI RO
        ...
        ELSE
          ...
          awg_in_is_hi = (DIGIN_LONG() AND AWG_done_DI_pattern)
      ...
      CASE 3 'MBI RO // go to SP-A (succes) OR Nitrogen reset OR CR check
        ...
        IF (counts >= MBI_threshold) THEN
          ...
          DIGOUT(AWG_event_jump_DO_channel,1)   ' AWG trigger (event jump)
          CPU_SLEEP(9)                                        ' need >= 20ns pulse width; adwin needs >= 9 as arg, which is 9*10ns
      ...
      CASE 5 'Start C13 MBI seq and wait for trigger // go to C13 MBI readout OR SP-A
        IF (timer=0) THEN
          DIGOUT(AWG_start_DO_channel,1)  ' send AWG trigger
          CPU_SLEEP(9)                                  ' need >= 20ns pulse width; adwin needs >= 9 as arg, which is 9*10ns
          DIGOUT(AWG_start_DO_channel,0)
      ...
      CASE 6 'C13 MBI readout // go to SP-A OR CR-Check
        ...
        IF(timer=0) THEN 'Start the laser
          ...
          CNT_CLEAR(counter_pattern)    'clear counter
          CNT_ENABLE(counter_pattern)    'turn on counter
          DAC(E_laser_DAC_channel, 3277*E_C13_MBI_RO_voltage+32768) ' turn on Ex laser
      ...
      CASE 7 'Spin pump A // go to next C13 MBI OR MBE OR final RO
        IF (timer = 0) THEN 'Turn on lasers
          DAC(A_laser_DAC_channel, 3277*A_SP_voltage_after_C13_MBI+32768) ' turn on A laser
          DAC(E_laser_DAC_channel, 3277*E_SP_voltage_after_C13_MBI+32768) ' turn on E laser
      ...
      CASE 9 'MBE RO // go to SP-A OR back to CR check
        ...
        IF (timer = 0) THEN 'Start the laser
          CNT_CLEAR(counter_pattern)     'clear counter
          CNT_ENABLE(counter_pattern)    'turn on counter
          DAC(E_laser_DAC_channel, 3277*E_MBE_RO_voltage+32768) ' turn on Ex laser
      ...
      CASE 10 'SP-A // go to more MBE steps OR final RO
        IF (timer = 0) THEN
          DAC(A_laser_DAC_channel, 3277*A_SP_voltage_after_MBE+32768) ' turn on A laser
          DAC(E_laser_DAC_channel, 3277*E_SP_voltage_after_MBE+32768) ' turn on E laser
      ...
      CASE 12 'parity RO // go to more parity msmnts OR final RO
        IF(timer=0) THEN 'Start the laser
          CNT_CLEAR(counter_pattern)    'clear counter
          CNT_ENABLE(counter_pattern)    'turn on counter
          DAC(E_laser_DAC_channel, 3277*E_Parity_RO_voltage+32768) ' turn on Ex laser

        ELSE
          counts = CNT_READ( counter_channel) ' Read counters
