...
LOWINIT:
  ...
  rnd_output_di_channel        = DATA_20[14]
  ...
  rnd_output_di_pattern = 2 ^ rnd_output_di_channel
  remote_CR_is_high = 0
  ...
  ELSE
    ...
    SELECTCASE mode
      ...
      case 4 ' trigger LDE, then wait for timeout (AWG) OR success (PLU)
        ...
        IF (timer = 5) THEN ' we need to make sure that the AWG is receptive for triggering now!
        ...
        ELSE
          ...
          IF ((PLU_was_high = 0) AND (PLU_is_high > 0)) THEN              
            ...
            mode = 0 '5 XXX no adwin RO
            timer = -1
            remote_mode = 0 '3 XXX no adwin RO
            first = 1
            first_local = 1
