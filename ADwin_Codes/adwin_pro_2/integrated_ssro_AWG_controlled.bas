...
DIM AWG_controlled_readout, counts, old_counts AS LONG
...
INIT:  
  ...
  AWG_controlled_readout       = DATA_20[13]
...
EVENT:
  ...
  ELSE
    ...
    SELECTCASE mode
      ...
      CASE 4    ' spin readout
        IF (AWG_controlled_readout = 0) THEN
          IF (timer = 0) THEN
            P2_CNT_CLEAR(CTR_MODULE, counter_pattern)    'clear counter
            P2_CNT_ENABLE(CTR_MODULE,counter_pattern)    'turn on counter
          ...
          ENDIF
        ELSE
          ' spin readout controlled by AWG
          IF (timer = 0) THEN
            P2_CNT_CLEAR(CTR_MODULE, counter_pattern)    'clear counter
            P2_CNT_ENABLE(CTR_MODULE,counter_pattern)    'turn on counter
            AWG_done = 0
