...
' modes:
'   0 : CR check
'   1 : safety wait-time for shutter (specified>35 msec)
...
' shutter stuff
DIM safety_wait_time AS LONG '35 ms to avoid ahutter open-close too frequently
DIM shutter_channel AS LONG
DIM wait_shutter_open AS LONG 'time to wait for shutter to be completely open/closed (around 1.5-2 msec)
    ...
    SELECTCASE mode
      ...
      CASE 5    '  wait for AWG sequence or for fixed duration
        ...
        if(send_AWG_start > 0) then
          ...
          IF (timer = wait_shutter_open) THEN
            P2_DIGOUT(DIO_MODULE,AWG_start_DO_channel, 1)  ' AWG trigger
            CPU_SLEEP(9)               ' need >= 20ns pulse width; adwin needs >= 9 as arg, which is 9*10ns
            P2_DIGOUT(DIO_MODULE,AWG_start_DO_channel, 0)          
          else
