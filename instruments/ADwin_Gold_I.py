...
class ADwin_Gold_I(Instrument): #1
    '''
    This is the driver for the Adwin Gold I

    Usage:
    Initialize with
    <name> = qt.instruments.create('name', 'ADwin_Gold_I', 'address = <device no>')
    ...
    def __init__(self, name, address): #2
         # Initialize wrapper
        logging.info(__name__ + ' : Initializing instrument ADwin Gold I')
...
#        self.add_function('Set_DAC_Voltage')
    ...
    def _load_dll(self): #3
        ...
        if ProcType != 1000:
            logging.warning(self.get_name() + ' WARNING: ADwin Gold I with T11 processor expected. Processor type %s found'%ProcType)
        if ErrorMsg.value != 0:
    ...
    def Boot(self):
        ErrorMsg=c_int32(0)
        filename = adwin_path+'\\ADwin9.btl'
