...
class JPE_CADM(Instrument):
    ...
    def __init__(self, name):
        ...
        #T: 		temperature in K: 0..300
        #type:		always set to PK1810
        #TRQFR:		torque amplification factor
        #freq:		operation frequency in Hz [1-600, integer values]
        #rel:		relative piezo step as percentage [1-100, integer values]
    ...
    def status(self, addr = 0):
        out = sp.check_output (['pzknb', 'S', str(addr)])
	...
	def info (self, addr, ch):
		out = sp.check_output (['pzknb', 'i', str(addr), str(ch)]
		print out
	...
	def move_cnt (self, cw = 0):
		if (cw == 0):
			...
			out = sp.check_output ['pzknb', 'M', str(addr), str(ch), self.type, self.T, 
	...
	def move (self, addr, ch, steps)
		...
		if not(steps==0):
			out = sp.check_output ['pzknb', 'M', str(addr), str(ch), self.type, self.T, 
											str(cw), str(self.freq), str(100), str(steps_int)]
			wait_time = (1./self.freq)*steps_int
			time.sleep(0.2+wait_time)
			out = sp.check_output ['pzknb', 'M', str(addr), str(ch), self.type, self.T, 
											str(cw), str(self.freq), str(1), str(steps_cent)]
