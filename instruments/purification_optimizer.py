...
class purification_optimizer(mo.multiple_optimizer):
    ...
    def auto_optimize(self):
        ...
        # Now wait. Yellow takes some time before getting to the optimized value
        # When the pid is switched back on, the setpoint is set to the current frequency
        # If you switch on the pids too early, yellow will never reach the optimized value
        qt.msleep(4.0)
        self.set_pidyellowfrq_running(True)
        self.set_pidgate_running(True)        
        self.set_pid_e_primer_running(e_primer_was_running)        
