...
class TimeHarp_TH260P(Instrument): #1
    '''
    This is the driver for the TimeHarp 260 PICO Time Correlated Single Photon Counting module.
    It is identical to that of the TimeHarp nano module except for the additional functions:
    TH260_SetTimingMode(DevNr, mode)
    Also, it has a CFD instead of a TTL discriminator

    Usage:
    Initialize with
    <name> = qt.instruments.create('name', 'TimeHarp_TH260P', DeviceIndex = [default:0] )
...
#        print ('HydraHarp DLL version %s loaded...'%str(LibraryVersion))
     ...
     #       print ('HydraHarp serial no. %s'%self.SerialNo)            
            ...
            logging.error('TH260P device could not be initialized.')
            self.CloseDevice()
...
#        print __name__ +' : Loading THLib.dll'
    ...
    def _do_set_InputCFD0(self, value,**kw):
        temp = self.Channel
        self.Channel = 0
        self.set_InputCFD(value,**kw)
        self.Channel = temp

    def _do_set_InputCFD1(self, value,**kw):
        temp = self.Channel
        self.Channel = 1
        self.set_InputCFD(value,**kw)
        self.Channel = temp

    def set_CFD(self, value, **kw):
        self._do_set_SyncCFD(value, **kw)
        self._do_set_InputCFD0(value,**kw)
        self._do_set_InputCFD1(value,**kw)

    def _do_set_InputCFD(self, value, zerox=CFDZERO):
        success = self._TH260.TH260_SetInputCFD(self.DevIdx, self.Channel, value, zerox)
        if success < 0:
            logging.warning(__name__ + ' : error in TH_SetInputCFD')
            self.get_ErrorString(success)

    def _do_set_SyncCFD(self, value, zerox=CFDZERO):
        success = self._TH260.TH260_SetSyncCFD(self.DevIdx, value, zerox)
        if success < 0:
            logging.warning(__name__ + ' : error in TH_SetSyncCFD')
            self.get_ErrorString(success)
