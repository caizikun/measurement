...
class auto_optimizer(Instrument):
    ...
    def __init__(self, name, get_freq_yellow = None, set_freq_yellow = None, get_counts_yellow = None, get_attemps_yellow = None,
        ...
        ins_pars  ={'do_plot'               :   {'type':types.BooleanType,'val':True,'flags':Instrument.FLAG_GETSET},
                    ...
                    'check_yellow_threshold':   {'type':types.FloatType,  'val':5,'flags':Instrument.FLAG_GETSET},                    
                    'check_newfocus_zeros'  :   {'type':types.IntType,    'val':4,'flags':Instrument.FLAG_GETSET},
                    ...
                    'opt_nf_good'           :   {'type':types.FloatType,  'val':100,'flags':Instrument.FLAG_GETSET},                                        
    ...
    def check_deionization(self):
        ...
        if (np.sum(datastream<0.00001) > self._NV0_zeros):
            return False;
        else:
            return True;         
    ...
    def check_detuned_repump(self):
        ...
        if (max(datastream) > self._opt_yellow_good) and (np.sum(datastream<0.00001) > self._check_newfocus_zeros):
            return True;
        else:
            return False;          
    ...
    def flow(self, state):
        ...
        1. Do you have low counts on gate? YES: go to 2. NO: go to 5.
        2. Are there counts at exactly 0 on gate? YES: go to 3. NO: go to 4.
        3. Yellow is off resonance. Optimize yellow, then go to 1.
        4. Gate is off resonance. Optimize gate, then go to 1.
        5. Do you have low counts on yellow? YES: go to 6. NO: go to 8.
        6. Do you have both zeros and high counts on yellow? YES: go to 7. NO: go to 3
        7. NewFocus is off resonance. Optimize NewFocus, then go to 5.
        8. Finished. 
        '''
        # Allow to break
        ...
        if state == 1:           
            ...
            print 'Low counts on yellow?'
            # Check if there are low counts on yellow
            if self.check_low_yellow():
                print 'YES'
                return self.flow(6);
            else:
                print 'NO'
    ...
    def optimize_newfocus(self):
        ...
        for currentNFStep,currentNFFreq in enumerate(nf_sweep):
            ...
            while (time.time() - NFStepTime <= self._opt_nf_dwell) and not finishedNF:
                # Allow to break
                if (msvcrt.kbhit() and (msvcrt.getch() == 'q')): 
                    self.set_freq_newfocus(nf_start);
                    print 'Quit by user'
                    break                
                # Get nf val
    ...
    def optimize_gate(self):  
        ...
        for currentGateStep,currentGateFreq in enumerate(gate_sweep):
            ...
            while (time.time() - gateStepTime <= self._opt_gate_dwell) and not finishedGate:
                ...
                if (sum(gate_y[-min(10,len(gate_y)):]<0.0001) <= self._NV0_zeros or len(gate_y) < 10):  
                    # Debug
                    ...
                    if np.mean(yellow_y[-min(5,len(yellow_y)):])>self._opt_yellow_good or finishedYellow or currentYellowStep == len(yellow_sweep): # Yellow is good. Continue gate
                        ...
                        while currentYellowStep < len(yellow_sweep) and (not finishedYellow) and (sum(gate_y[-min(10,len(gate_y)):]<0.0001) > self._NV0_zeros):                           
                            # Allow to break
                            if (msvcrt.kbhit() and (msvcrt.getch() == 'q')): 
                                self._set_freq_yellow(yellow_start);
                                self._set_freq_gate(gate_start);                
                            ...
                            while (time.time() - yellowStepTime <= self._opt_yellow_dwell) and not finishedYellow and (sum(gate_y[-min(10,len(gate_y)):]<0.0001) > self._NV0_zeros):
                                # Allow to break
                                if (msvcrt.kbhit() and (msvcrt.getch() == 'q')): 
                                    self._set_freq_yellow(yellow_start);
                                    self._set_freq_gate(gate_start);                
        ...
        if finishedYellow:
            print 'Yellow: good value during scan, at', max(yellow_y)/self._opt_yellow_good, 'times threshold at frequency', yellow_x[np.argmax(yellow_y)];
        else:
            self._set_freq_yellow(yellow_x[np.argmax(yellow_y)]);
            print 'Yellow: scan failed, set to best value ', max(yellow_y), 'at frequency', yellow_x[np.argmax(yellow_y)];
    ...
    def optimize_yellow(self):  
        ...
        for currentYellowStep,currentYellowFreq in enumerate(yellow_sweep):
            ...
            while (time.time() - yellowStepTime <= self._opt_yellow_dwell) and not finishedYellow:
                ...
                if (sum(gate_y[-min(10,len(gate_y)):]<0.0001) > 1 or len(gate_y) < 10):  
                    ...
                    if np.mean(gate_y[-min(5,len(gate_y)):])>self._opt_gate_good or finishedGate or currentGateStep == len(gate_sweep): # Gate is good. Continue yellow
                        # Debug
                        ...
                        while currentGateStep < len(gate_sweep) and not finishedGate and (sum(gate_y[-min(10,len(gate_y)):]<0.0001) <= self._NV0_zeros):                           
                            # Allow to break
                            if (msvcrt.kbhit() and (msvcrt.getch() == 'q')): 
                                self._set_freq_yellow(yellow_start);
                                self._set_freq_gate(gate_start);                
                            ...
                            while (time.time() - gateStepTime <= self._opt_gate_dwell) and not finishedGate and (sum(gate_y[-min(10,len(gate_y)):]<0.0001) <= self._NV0_zeros):
                                # Allow to break
                                if (msvcrt.kbhit() and (msvcrt.getch() == 'q')): 
                                    self._set_freq_yellow(yellow_start);
                                    self._set_freq_gate(gate_start);                
        ...
        if finishedGate:
            print 'Gate: good value during scan, at', max(gate_y)/self._opt_gate_good, 'times threshold at frequency', gate_x[np.argmax(gate_y)];
        else:
            self._set_freq_gate(gate_x[np.argmax(gate_y)]);
            print 'Gate: scan failed, set to best value ', max(gate_y), 'at frequency', gate_x[np.argmax(gate_y)];
