...
class simple_optimizer(Instrument):
    ...
    def __init__(self, name, get_control_f=None, set_control_f=None, get_value_f=None, get_norm_f=None, plot_name='', scan_mode='smooth'):
    ...
    def scan(self):
        ...
        print self._scan_mode
        if self._scan_mode == 'unidirectional':
            print 'performing unidirectional scan'                                 
            udrange = np.linspace(scan_min, scan_max, steps)
        elif self._scan_mode == 'bidirectional':
            print 'performing bidirectional scan'                                 
            udrange = np.linspace(scan_min, scan_max, steps)
            udrange= np.append( udrange, np.linspace(scan_max, scan_min, steps) )
        else: 
            print 'performing smooth scan'
